...
static uint64_t bitReverse8(const uint64_t value) {
  return (reverse4[value & 0x0f] << 4) | reverse4[(value >> 4) & 0x0f];
}
static uint64_t bitReverse16(const uint64_t value) {
  return (bitReverse8(value & 0xff) << 8) | bitReverse8(value >> 8);
}
static uint64_t bitReverse32(const uint64_t value) {
  return (bitReverse16(value & 0xffff) << 16) | bitReverse16(value >> 16);
}
static uint64_t bitReverse64(const uint64_t value) {
  return (bitReverse32(value & 0xffffffff) << 32) | bitReverse32(value >> 32);
}
uint64_t bitReverse(const uint64_t value, const unsigned bits) {
  ...
  return bitReverse64(value) >> (64 - bits);
}
