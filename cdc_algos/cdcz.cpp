...
CdcCandidatesResult find_cdc_cut_candidates(std::span<uint8_t> data, const uint32_t min_size, const uint32_t avg_size, const uint32_t max_size, const CDCZ_CONFIG& cdcz_cfg, bool is_first_segment) {
  ...
  if (
    ...
    cdc_find_cut_points_with_invariance(
...
void cdc_find_cut_points_with_invariance(
  ...
  auto process_lane = [&lane_results, &lane_achieved_chunk_invariance, &min_size, &avg_size, &max_size, &bytes_per_lane, &vindex_max,
    ...
    compute_features = cdcz_cfg.compute_features, use_supercdc_backup_mask = cdcz_cfg.use_supercdc_backup_mask]
    ...
    auto adjust_lane_for_jump = [&minmax_adjustment_vec, &backup_cut_vec, &max_size_vec, &jump_vec, &vindex, &min_size,
      use_fastcdc_subminimum_skipping = cdcz_cfg.use_fastcdc_subminimum_skipping, use_supercdc_minmax_adjustment = cdcz_cfg.use_supercdc_minmax_adjustment, use_supercdc_backup_mask = cdcz_cfg.use_supercdc_backup_mask]
      (const uint8_t lane_i) {
      int32_t new_lane_pos = _mm256_extract_epi32_var_indx(jump_vec, lane_i);
      if (use_fastcdc_subminimum_skipping) {
      ...
      while (true) {
        ...
        while (j < 4) {
          ...
          if (!cdcz_cfg.use_supercdc_backup_mask || lane_has_result != 0) {
            if (cdcz_cfg.use_fastcdc_normalized_chunking) {
            ...
            if (!cdcz_cfg.use_fastcdc_normalized_chunking || lane_has_result != 0) {
